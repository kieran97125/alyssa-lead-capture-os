...
function resolveProjectRef() {
  ...
  const hostname = new URL(required("NEXT_PUBLIC_SUPABASE_URL")).hostname;
...
function validateRecipientFacingTemplate(name, html, expectedType) {
  assert.match(html, /https:\/\/app\.beautytrialhk\.com\/auth\/confirm\?/, `${name} must open on the canonical Growth OS domain.`);
...
async function requestConfig(endpoint, accessToken, init = {}) {
  ...
  if (!response.ok) throw new Error(`Supabase Management API request failed (${response.status} ${response.statusText}).`);
  return response.json();
}
const accessToken = required("SUPABASE_ACCESS_TOKEN");
const projectRef = resolveProjectRef();
if (!/^[a-z0-9]+$/i.test(projectRef)) throw new Error("SUPABASE_PROJECT_REF has an invalid format.");
const [inviteHtml, magicLinkHtml] = await Promise.all([readTemplate("invite.html"), readTemplate("magic-link.html")]);
...
// Deliberately scoped PATCH: no account, password, session, SMTP, redirect,
// signup, MFA or authorization settings are rewritten by this operation.
const expected = {
...
await requestConfig(endpoint, accessToken, { method: "PATCH", body: JSON.stringify(expected) });
const verified = await requestConfig(endpoint, accessToken);
for (const [key, value] of Object.entries(expected)) assert.equal(verified[key], value, `Hosted Auth config mismatch: ${key}`);
const digest = value => createHash("sha256").update(value).digest("hex").slice(0, 12);
console.log(JSON.stringify({
  ok: true,
  synced: ["invite", "magic_link"],
  leakedPasswordProtection: verified.password_hibp_enabled === true,
