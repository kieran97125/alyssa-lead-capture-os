...
collaboration_component = r'''"use client";
...
'''
write("src/components/creative/CreativeJobCollaborationDialog.tsx", collaboration_component)
...
studio = replace_once(
  ...
  UserRound,
    ...
    "CreativeJobStudio icon imports",
)
studio = replace_once(
    studio,
    '''import { ConfirmSubmitButton } from "@/components/alyssa/ConfirmSubmitButton";
...
import { CreativeBriefEditor } from "@/components/creative/CreativeBriefEditor";''',
    "CreativeJobStudio editor imports",
)
studio = replace_once(
    studio,
...
import {
  ...
  updateCreativeJobStatusAction,
    ...
    "CreativeJobStudio collaboration imports",
)
studio = replace_once(
    studio,
    '''  creativeAssetPurposeLabels,
  ...
  creativeJobStatusLabels,''',
    '''  creativeJobStatusLabels,''',
    "CreativeJobStudio unused asset constants",
)
studio, removed = re.subn(
    r"\nfunction assetIcon\(asset: CreativeAsset\) \{[\s\S]*?\n\}\n",
    "\n",
    studio,
    count=1,
)
if removed != 1:
    raise RuntimeError(f"CreativeJobStudio assetIcon removal: found {removed}")
studio = replace_once(
    studio,
    ...
    '''              <CreativeJobCollaborationDialog
              ...
              <CreativeBriefHistoryDialog
                jobId={props.job.id}''',
    "CreativeJobStudio on-demand collaboration trigger",
)
...
fixture = replace_once(
  ...
  CreativeComment,
  CreativeDesignerProfile,''',
    "Creative fixture asset/comment type imports",
)
fixture_anchor = '''const fixtureVersions: CreativeBriefVersion[] = ['''
fixture_insert = '''const fixtureAssets: CreativeAsset[] = [
  {
...
if fixture.count(fixture_anchor) != 1:
    raise RuntimeError("Creative fixture versions anchor missing")
fixture = fixture.replace(fixture_anchor, fixture_insert + fixture_anchor, 1)
fixture = replace_once(
    fixture,
    '''            assets={[]}
            comments={[]}''',
    ...
    "Creative fixture collaboration data",
)
...
e2e = replace_once(
    ...
    "Creative e2e test title",
)
collaboration_test = r'''
...
'''
mobile_anchor = '\ntest("creative workspace remains usable on mobile without horizontal page overflow"'
if e2e.count(mobile_anchor) != 1:
    raise RuntimeError("Creative e2e mobile anchor missing")
e2e = e2e.replace(mobile_anchor, collaboration_test + mobile_anchor, 1)
write(e2e_path, e2e)
...
const editorStyles''',
    "Creative contract collaboration file",
)
contract = replace_once(
    contract,
...
assert.match(studio, /creative-job-settings-form/);''',
    "Creative contract collaboration usage",
)
contract = replace_once(
    contract,
...
assert.match(collaborationDialog, /Brief Screenshot 只作解釋/);''',
    "Creative contract collaboration actions",
)
...
learning = learning.replace(
    "Version history is available through a compact same-page side sheet; asset/comment records and backend actions remain intact but are not permanent workspace panels.",
    "Version history is available through a compact same-page side sheet. Production deliverables and revision discussion remain available through a second on-demand collaboration sheet, while neither workflow permanently occupies the writing surface.",
)
learning = learning.replace(
...
learning = learning.replace(
    "inline-explanation versus production-asset separation; version history as an on-demand surface; readable operational density.",
    "inline-explanation versus production-asset separation; version and collaboration history as on-demand surfaces; readable operational density.",
)
learning = learning.replace(
...
print("Implemented on-demand Creative deliverable and discussion handoff.")
