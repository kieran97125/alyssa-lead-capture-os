...
- CRM Phase 3.7A prepares the final apply-ready editable settings migration at `docs/CRM_PHASE_3_7_APPLY_EDITABLE_SETTINGS.sql`. The SQL has not been executed yet; `crm_app_settings` will enable future editable CRM settings, while code defaults remain the fallback.
- CRM Phase 3.8 prepares a non-destructive default settings seed at `docs/CRM_PHASE_3_8_SEED_DEFAULT_SETTINGS.sql`. The seed is not executed by the app, uses `on conflict do nothing`, and keeps code defaults as the fallback while DB-backed settings are tested.
...
- `docs/CRM_PHASE_3_7_APPLY_EDITABLE_SETTINGS.sql` is the final apply-ready CRM editable settings migration prepared for review. It has not been executed.
- `docs/CRM_PHASE_3_8_SEED_DEFAULT_SETTINGS.sql` is the non-destructive CRM settings default seed prepared for manual review. It has not been executed by this app change.
