...
For mobile Wix performance, keep treatment-page sections practical: avoid stacking multiple large decorative cards before the form, hide non-essential decorative images on mobile, lazy-load below-the-fold images, and keep the Wix HTML Embed wrapper full-width without a narrow fixed mobile box. The LaunchHub embed script version `20260626-no-default-form-flash` is the recommended version for the flat mobile form UI, lighter pre-resize iframe height, and neutral loading state before the requested form config is resolved.
