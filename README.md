...
- CRM Phase 4.0 enables the first editable settings save flow for `quick_replies` only. `/crm/settings` now uses a section-based settings layout with left navigation and only the active section rendered on the right, replacing the long-scroll settings dump. Quick Replies remain a compact row-based editor and can update titles/message bodies through server-only admin DB access; all other settings remain read-only, WhatsApp remains manual open only, and no auto-send or external AI API is connected.
- CRM Phase 4.1 corrects the reply workflow direction: Quick Replies and AI Assist are separate tools that both feed the lead detail Reply Composer. Quick Replies insert preset replies into an editable message box, AI Assist generates rule-based local suggestions for review, and the future WhatsApp send button remains disabled; no WhatsApp API, external AI API, auto-send, or Meta sending is connected.
...
<script
  src="https://go.beautytrialhk.com/embed/alyssa-form.js?v=stable"
...
- Alyssa should use `operator_name = Alyssa Group Limited`, `privacy_url = https://www.alyssa.hk/privacy`, and `disclaimer_url = https://www.alyssa.hk/disclaimer`.
- If a brand has separate `privacy_url` / `disclaimer_url` and no `legal_page_url`, public forms and public Landing Pages show those separate legal links.
- If a brand does not have `legal_page_url`, `privacy_url`, or `disclaimer_url`, LaunchHub keeps the existing fallback links for Privacy Policy, Terms & Conditions, and Disclaimer.
- Review and apply `docs/APPLY_BRAND_LEGAL_SETTINGS_REVIEW.sql` in Supabase before expecting separate Privacy / Disclaimer URL edits to persist in the database.
